...
class Gear_desc extends CI_Controller {
	...
	public function index()
	{
		...
		Left JOIN ks_users c ON e.create_user = c.app_user_id 
...
{   
	...
	public function save()
	{
		...
		//print_r($this->input->post('unavailble_dates'));
	...
	if (empty($this->input->post('sub_category_name'))) {
		...
		if($this->form_validation->run())
		{
			...
			if ($sub_category_id == '') {
				...
				$row['ks_category_id'] =  $sub_category_id;
			}
			//pri
		...
		$row['owners_remark']= substr(trim($this->input->post('owner_remark')),3,-4);
		...
		$row['per_weekend_cost_aud_ex_gst']= $this->input->post('per_weekend_cost_aud_ex_gst');
		$row['per_weekend_cost_aud_inc_gst']= $this->input->post('per_weekend_cost_aud_ex_gst') +( $this->input->post('per_weekend_cost_aud_ex_gst')*$gst_rate);
		$row['per_week_cost_aud_ex_gst']= $this->input->post('per_week_cost_aud_ex_gst');
		$row['per_week_cost_aud_inc_gst']= $this->input->post('per_week_cost_aud_ex_gst') +( $this->input->post('per_week_cost_aud_ex_gst')*$gst_rate);
		...
		if (count($_FILES['images']) > 0 ) {
				    ...
				    {           
				       ...
				       // print_r($dataInfo['file_name']);die;
				         $data_images = array(

				         					'model_id' => $model_id,
				         					'gear_display_image' =>$dataInfo['file_name'] ,
				         					'user_gear_desc_id'=> $value,
				         					'is_active'=> 'Y',
				         					 'create_user'=> $this->session->userdata('ADMIN_ID'),
				         					 'create_date'=>date('Y-m-d')
	...
	public function edit()
	{
		...
		$models=$this->common_model->GetAllWhere("ks_gear_unavailable",array("user_gear_description_id"=>$id));
...
//////////////////////////////Pagination config//////////////////////////////////				
	...
	public function update()
	{
		...
		if(count($this->input->post('unavailble_dates')) > 0 ) {
			...
			foreach($this->input->post('unavailble_dates') AS $unavailble_dates){
				...
				$to_date_array =  explode('/',$date_array[1]);
				 $to_date =  trim($to_date_array[2]).'-'.trim($to_date_array[0]).'-'.trim($to_date_array[1]);
				 echo $ks_gear_unavailable_id = $_POST['ks_gear_unavailable_id'][$i];
	...
	{
		$query = $this->common_model->GetAllWhere('ks_user_gear_description' , array('app_user_id'=>$app_user_id , 'gear_type!='=> '3'));
	...
	public function getmodalDetails()
	{
		...
		if(count($result_arr) > 0 ){
			if($result_arr->model_image != ''){
				$result_arr->model_image = 'http://inferasolz.com/kitshare/'.'/site_upload/gear_images/model_images/'.$result_arr->model_image ;
			}
			...
			if(count($result_arr) > 0){
				...
				if($result_arr->model_image != ''){
				$result_arr->model_image = 'http://inferasolz.com/kitshare/'.'site_upload/model_images/'.$result_arr->model_image ;
			}
			$result['status'] = '200';
			$result['status_message'] = 'Success';
			$result['match_found'] = 1;
