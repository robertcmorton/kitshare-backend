...
class Gear_desc extends CI_Controller {
	...
	public function index()
	{
		...
		".$where." ORDER BY e.user_gear_desc_id DESC";
	...
	public function add()
	{
		...
		if (!empty($category)) {
			...
			foreach ($category as  $value) {
				$value_category[]  =  htmlspecialchars_decode($value->gear_category_name,ENT_QUOTES) ;
			}
		}
		...
		if (!empty($category_sub)) {
			...
			foreach ($category_sub as  $value) {
				$value_category_sub[]  =  htmlspecialchars_decode($value->gear_category_name , ENT_QUOTES)  ;
			}
		}
		...
		$sql ="SELECT app_user_id , app_user_first_name ,app_user_last_name FROM  ks_users WHERE  is_active='Y' "; 
...
{   
    ...
    $config = array();
    $config['upload_path'] = ROOT_UPLOAD_IMPORT_PATH.'/site_upload/gear_images_original/';
...
public function resize($image_data) {
	...
	public function save()
	{
		...
		//check category id 
		$query =$this->common_model->GetAllWhere('ks_gear_categories',array('gear_category_name' => htmlspecialchars($this->input->post('category_name'))));
		...
		if (!empty($category_data)) {
			...
			'gear_category_name' => htmlspecialchars($this->input->post('category_name')),
	...
	if (empty($this->input->post('sub_category_name'))) {
		...
		$query =$this->common_model->GetAllWhere('ks_gear_categories',array('gear_category_name' => htmlspecialchars($this->input->post('sub_category_name'))));
		$category_data = $query->row();
		if (!empty($category_data)) {
			...
			$category_insert = array(
			'gear_category_name' => htmlspecialchars($this->input->post('sub_category_name')),
		...
		$slug_name =  $this->alphanumericAndSpace(trim($this->input->post('gear_name')));
		...
		$row['gear_hide_search_results'] = $this->input->post('gear_hide_search_results');
		...
		//Record is inserted into the ks_user_gear_search
		$search_array = array();
		$search_array = $row;
		$search_array['model_name'] = $this->input->post('modal_name');
		$search_array['manufacturer_name'] = $this->input->post('manufacturer_name');
		$search_array['user_gear_desc_id']=$value;
		$search_array['gear_category_name']=htmlspecialchars($this->input->post('category_name'));
		$search_array['gear_subcategory_name']=htmlspecialchars($this->input->post('sub_category_name'));
		
		$this->common_model->addRecord('ks_user_gear_search',$search_array);
		///////////////////////

		$this->db->where('user_gear_desc_id', $value);
		$this->db->update('ks_user_gear_description', array('gear_slug_name'=>$slug_name.'-'.$value)); 
				    ...
				    {           
				        ...
				        if ($image_data['image_width'] > '300' || $image_data['image_width'] > 300 ) {
				        	...
				        	$image =  $dataInfo['file_name'] ;
				        	  $imagePath =  UPLOAD_IMAGES.'site_upload/gear_images_original/'.$image;
								$newPath = UPLOAD_IMAGES.'/site_upload/gear_images/';
								$ext = '.jpg';
								$newName  = $image;
				        	  ...
				        	  $configer =  array(
					              'image_library'   => 'gd2',
					              'source_image'    =>  UPLOAD_IMAGES.'/site_upload/gear_images/'.$image,
					              'maintain_ratio'  =>  true,
					              'width'           =>  IMG_WIDTH,
					              'height'          =>  IMG_HEIGHT,
					            );
	  ...
	  //        				);
		...
		if (count($feature_details_id) > 0 ) {
			...
			if ($this->db->trans_status() === FALSE)
			{
					...
					$message = '<div class="alert alert-danger">Something went wrong! Please try again.</p></div>';
					$this->session->set_flashdata('success', $message);
					redirect('gear_desc/add');
			}
			else
			{
...
//////////////////////////////Pagination config//////////////////////////////////				
	...
	public function update()
	{
		...
		if(count($this->input->post('unavailble_dates')) > 0 ) {
			...
			foreach($this->input->post('unavailble_dates') AS $unavailble_dates){
				...
				if(strstr($unavailble_dates,"-")){
					$date_array =  explode( '-' ,$unavailble_dates);
								
					
					if(strstr($date_array[0],"/")){
						$from_date_array =  explode('/',$date_array[0]);
						$from_date =  trim($from_date_array[2]).'-'.trim($from_date_array[0]).'-'.trim($from_date_array[1]);
						$to_date_array =  explode('/',$date_array[1]);
						$to_date =  trim($to_date_array[2]).'-'.trim($to_date_array[0]).'-'.trim($to_date_array[1]);
						
						if(array_key_exists("ks_gear_unavailable_id",$_POST))
		...
		if($this->form_validation->run() == true )
		{
			...
			if(count($this->input->post('address_id')) > 0 ){
				...
				$address_id_arr = $this->input->post('address_id');
				$location_query =$this->common_model->GetAllWhere('ks_gear_location',array('user_gear_desc_id' => $id));
				$gear_location_data = $location_query->row();
				
				if(count($gear_location_data)>0){
					//Record is deleted
					$this->common_model->delele('ks_gear_location','user_gear_desc_id',$id);					
				}
				
				foreach($address_id_arr as $value){
					//Gear Location table is updated
					$address_insert =  array(
		...
		$row['gear_slug_name'] =  $slug_name.'-'.$id;
		...
		$row['update_date']= date('Y-m-d') ;
		$row['model_name'] = $this->input->post('modal_name');
		$row['manufacturer_name'] = $this->input->post('manufacturer_name');
		$row['gear_category_name'] = htmlspecialchars($this->input->post('category_name'));
		$row['gear_subcategory_name'] = htmlspecialchars($this->input->post('sub_category_name'));
		
		$this->db->where('user_gear_desc_id', $id);
		$this->db->update('ks_user_gear_search', $row); 
		///////////////////
	  ...
	  //        				);
		...
		{
				$this->db->trans_rollback();
				$message = '<div class="alert alert-danger"><p>Something went wrong! Please try again.</p></div>';
				$this->session->set_flashdata('success', $message);
				redirect('gear_desc');
		}
		else
	...
	public function delete_record()
	{
		...
		$this->common_model->delele('ks_user_gear_search','user_gear_desc_id',$id);
	...
	// }
		public function getsubcategory()
	{
		$category_id = htmlspecialchars($this->input->post('category_id'),ENT_QUOTES);
		$data= array();
		$query = $this->common_model->GetAllWhere('ks_gear_categories',array('is_active', 'gear_category_name'=>$category_id));
		$gear_name = $query->row();
		$query = $this->common_model->GetAllWhere('ks_gear_categories',array('is_active', 'gear_sub_category_id'=>$gear_name->gear_category_id));
	...
	public function UpdateImageSize(){
		...
		$directory = UPLOAD_IMAGES."site_upload/gear_images_original/";
			$images = glob($directory . "/*");
			$this->load->library('upload');
			foreach($images as $image)
			{
			  // echo $image;die;
			  $images =  explode('gear_images_original//', $image);
			  // print_r($images[1]);
			  $images_change = $images[1] ;
			  // echo "<br>";die;
			  $imagePath =  UPLOAD_IMAGES.'site_upload/gear_images_original/'.$images_change;
				$newPath = UPLOAD_IMAGES.'/site_upload/gear_images/';
				$ext = '.jpg';
				$newName  = $image;
			  ...
			  $configer =  array(
			              'image_library'   => 'gd2',
			              'source_image'    =>  UPLOAD_IMAGES.'/site_upload/gear_images/'.$images_change,
			              'maintain_ratio'  =>  true,
			              'width'           =>  IMG_WIDTH,
			              'height'          =>  IMG_HEIGHT,
			            );
