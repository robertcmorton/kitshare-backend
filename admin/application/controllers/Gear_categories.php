...
class Gear_categories extends CI_Controller {
	...
	public function index()
	{
		...
		$data['gear_category_name']		  = $this->input->get('gear_category_name');
		...
		$result=$sql->result();	
		if($id!="")
			$base_url = base_url()."gear_categories/index/".$id."?gear_category_name=".$data['gear_category_name']."";
		else
			$base_url =  base_url()."gear_categories?gear_category_name=".$data['gear_category_name']."";
...
//////////////////////////////Pagination config//////////////////////////////////				
		...
		$this->load->view('gear_categories/list', $data);
	...
	public function save()
	{
		...
		$this->form_validation->set_rules($this->validation_rules['Add']);
			...
			if($this->form_validation->run())
			{
				...
				$row['gear_category_name']= htmlspecialchars($this->input->post('gear_category_name'),ENT_QUOTES);
				$row['average_value']=  htmlspecialchars($this->input->post('gear_category_name'),ENT_QUOTES);
				...
				//checked whether the gear category name already exists in the database or not
				$where_array = "is_active='Y' AND (`gear_sub_category_id`=".$row['gear_sub_category_id']." AND `gear_category_name`='".$row['gear_category_name']."')";
				$gear_categories=$this->common_model->GetAllWhere("ks_gear_categories",$where_array);
				
				if($gear_categories->num_rows()>0){
				
					$message = '<div class="alert alert-danger">Gear Category already exists!</p></div>';
	...
	public function edit()
	{
		...
		if($categories->num_rows()>0)		
			...
			$categories=$this->common_model->GetAllWhere("ks_gear_categories",$where_array);
			$data['result'] = $categories->result();
		}
		...
		$this->load->view('common/header');	
		$this->load->view('common/left-menu');	
		$this->load->view('gear_categories/edit', $data);
		$this->load->view('common/footer');		
	}
	...
	public function get_subcategory(){
		...
		$where_array = array('gear_sub_category_id'=>$cat_id,'is_active'=>'Y');
		$subcat_result=$this->common_model->GetAllWhere("ks_gear_categories",$where_array);
						...
						while (($importdata = fgetcsv($file, 10000, ",")) !== FALSE)
						{
								...
								if($count>0){
									...
									for($i=0;$i<$cnt;$i++){
										...
										if($i==0){
											$categories['gear_category_name'] = $importdata[$i];
	...
	public function downloadallcsv(){
		...
		$this->db->select('a.gear_category_id AS CATEGORY_ID,a.gear_category_name AS CATEGORY_NAME,a.gear_sub_category_id AS PARENT_CATEGORY_ID,a.is_active AS STATUS,a.average_value,a.security_deposit');
