...
class Upload_models extends CI_Controller {
    ...
    public function save() {
        ...
        if ($this->input->post()!='') {
         	...
         	$path = ROOT_UPLOAD_IMPORT_PATH .'site_upload/models_excel/';
          
 			//	echo	$_SERVER['DOCUMENT_ROOT'].preg_replace('@/+$@','',dirname($_SERVER['SCRIPT_NAME'])).'/';die;
            ...
            if ($flag == 1) {
                ...
                for ($i = 2; $i <= $arrayCount; $i++) {
					...
					$manufacturer = htmlspecialchars(trim($allDataInSheet[$i][$manufacturer]),ENT_QUOTES);
					$model = htmlspecialchars(trim($allDataInSheet[$i][$model]),ENT_QUOTES);
					if($model!=""){
							...
							$category = htmlspecialchars(trim($allDataInSheet[$i][$category]),ENT_QUOTES);
							$subcategory = htmlspecialchars(trim($allDataInSheet[$i][$subcategory]),ENT_QUOTES);
						   ...
						   //cheacking whether the Gear Category is empty or not
							if($category!=''){
								...
								if($subcategory!=''){
									//duplicate checking for Manufacturers
									$query_sub_category = $this->common_model->GetAllWhere("ks_gear_categories",array("gear_category_name"=>$subcategory));
									if($query_sub_category->num_rows()>0){
										$result_sub_category = $query_sub_category->result();
										$gear_sub_category_id = $result_sub_category[0]->gear_category_id;
						   ...
						   //cheacking whether the Model is empty or not
							...
							if( $category!=''  && $subcategory !=''){		
									$row_model['model_name']= htmlspecialchars($model,ENT_QUOTES);
									$row_model['model_description'] = $description;
									$row_model['gear_category_id'] = $gear_category_id;
									$row_model['gear_sub_category_id'] = $gear_sub_category_id;
									...
									if($action == "update"){	
										//echo $action;
										$row_model['gear_sub_category_id'] = $subcategory;
										$row_model['gear_category_id'] = $category;
										$row_model['manufacturer_id'] = $manufacturer;
										$repeated[]= $row_model ;
		...
		$this->load->view('common/header');	
		$this->load->view('common/left-menu');	
		$this->load->view('models/error',$data);
		$this->load->view('common/footer');	
