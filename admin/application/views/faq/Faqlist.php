    ...
    <div class="card">
      <div class="card-body">
        ...
        <h6 class="card-subtitle">(
          <?php if($total_rows>1){ echo $total_rows." Faq"; }else{ echo $total_rows." Faq"; }?>
          )</h6>
            ...
            <div class="has-feedback">
              <form class="form-inline example example53" data-toggle="validator" role="form" method="get" action="<?php echo base_url();?>gear_desc">
               <!--  <div class="form-group col-md-10 no_padding">
                ...
                <div class="form-group col-md-2 pull-right"> <a href="<?php echo base_url();?>Faq/faq_query" class="btn btn-primary pull-right add_userbtn">Add FAQ  &nbsp;<i class="fa fa-plus" aria-hidden="true"></i></a> </div>
              </form>
              <!-- </div> -->
              <!-- </div> -->
            </div>
        ...
        <!-- /.box-header -->
        <form name="bulk_action_form" action="<?php echo base_url();?>faq/select_delete_faq" method="post" onSubmit="return delete_confirm();"/>
            ...
            <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="150%" style="align:center;table-hlayout:fixed;">
              <thead>
                <tr>
                  ...
                  <th class="font_n">#</th>
                  <th class="font_n">Category Name</th>
                  <th class="font_n">Title</th>
                  <th class="font_n">Image</th>
                  <th class="font_n">Order By</th>
              ...
              <tbody>
                ...
                <tr>
                  <td><input type="checkbox" name="checked_id[]" class="checkbox" id="checked_id" value="<?php echo $row->faq_id;?>"></td>
                  <td class="mailbox-name"><?php echo $i;?></td>
                  <td class="mailbox-subject"><?php echo $row->category_name;?></td>
                  <td class="mailbox-subject"><?php echo $row->title;?></td>
                  <td class="mailbox-subject"><?php if(!empty($row->image)){?><img src="<?php echo ROOT_PATH1.'site_upload/faq_images/'.$row->image;?>" height="100" width="100"> <?php }else{ } ?></td>
                  <td class="mailbox-subject details" >
                    <input type="hidden" name="faq_id" class="faq_id" value="<?php echo $row->faq_id; ?>">  
                    <input type="number" style="width: 63px;" name="order_by" class="order_by" value="<?php echo $row->order_by; ?>">
                  </td>
                  ...
                  <td class="mailbox-date">
            				  <!-- <a href="<?php echo base_url();?>Faq/view/<?php echo $row->id; ?>" title="View"><i class="fa fa-eye" aria-hidden="true"></i></a> -->
            				  <a href="<?php echo base_url();?>Faq/editfaq/<?php echo $row->faq_id; ?>" title="Modify"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a> 
                      <a onClick="return confirm('Would You Like To Delete This?');" href="<?php echo base_url();?>Faq/delete_record_faq/<?php echo $row->faq_id; ?>" title="Delete"><i class="fa fa-trash-o"></i></a>
                  </td>
                </tr>
                <?php $i++; }   ?>
                <?php } else {?>
...
$(function() {
    $("#limit").change(function() {
		...
		window.location.href = "<?php echo base_url() ?>Faq/Faqlist/?limit="+limit;
...
$('.details').change(function(){
 var order_by = $(this).closest('tr').find('td .order_by').val();
 var faq_id = $(this).closest('tr').find('td .faq_id').val();
   $.ajax({
            type:'Post',
            url: '<?php echo base_url(); ?>Faq/faqOrderByUpdate',
            data :{order_by : order_by ,faq_id :faq_id},
            dataType:'Json',
            success: function(data){
                console.log(data);
                window.location.href = "<?php echo base_url() ?>Faq/faqlist";
            }
         });  
