...
class Home_model extends CI_Model {
	...
	{
		$this->db->select('a.gear_name,a.is_active,
    ...
    public function GearCategories($app_user_id,$where, $limit='',$offset='',$order_by='')
	{
	  ...
	  $this->db->select('ks_user_gear_description.user_gear_desc_id,ks_user_gear_description.gear_slug_name,
		...
		if (!empty($result_categories)) {
			...
			// echo "<pre>";	
			// print_r($result_categories);die;
		if (!empty($where)) {
			$gears=$this->GearListConditons($app_user_id,$where,$limit ,$offset,$order_by );
		}else{
			$gears=$this->GearList($app_user_id,$limit ,$offset,$order_by );
			//die;
		}	
   ...
   public function geratDetails($gear_id='')
   {
		...
		$this->db->order_by('user_gear_image_id','ASC');
		$query = $this->db->get();
   ...
   public function getModelsList($where_array,$limit=0,$offset=0,$order_by_fld='',$order_by=''){
	...
	$this->db->select('a.user_gear_desc_id,a.gear_slug_name,
		...
		if ($this->input->get('address') ||$this->input->get('suburb_name') ) {
	...
	public function getUserCart1($app_user_id,$order_id='')
	{
			...
			$this->db->select('cart.user_gear_rent_detail_id,cart.ks_insurance_category_type_id,cart.user_gear_desc_id,cart.user_gear_rent_id,cart.security_deposit,cart.owner_insurance_amount,
						...
						c.app_user_first_name,c.app_user_id,gear.user_address_id,
		...
		public function getUserCartbyOrderId($order_id)
	{
		...
		$this->db->select('cart.order_id,cart.create_user,cart.user_gear_rent_detail_id,cart.user_gear_desc_id,cart.user_gear_rent_id,cart.security_deposit,
		cart.gear_rent_requested_on,cart.gear_rent_request_from_date,cart.gear_rent_request_to_date,
		cart.gear_total_rent_request_days,cart.total_rent_amount_ex_gst,cart.gst_amount,cart.other_charges,cart.total_rent_amount,a.gear_name,
						a.per_day_cost_aud_ex_gst,cart.project_name,
						a.per_day_cost_aud_inc_gst,
						...
						a.owner_app_user_last_name As app_user_last_name,c.primary_email_address,a.is_registered_for_gst  AS registered_for_gst,c.user_profile_picture_link,a.owner_app_show_business_name  AS owner_show_business_name,a.owner_app_bussiness_name As  owner_bussiness_name,  
						a.renter_app_user_first_name As renter_firstname,renter_users.app_user_id As renter_app_user_id,
	...
	public function zipcodeRadiusLatLon($lngWest,$lngEast,$latSouth,$latNorth){	
		$sql = "SELECT distinct(postcode) FROM `ks_user_address`  WHERE `lat` <= '".$latNorth."' AND `lat` >= '".$latSouth."' AND `lng` >= '".$lngWest."' AND `lng` <= '".$lngEast."'";
	...
	public function OwnerReviewList($where)
	{
		...
		$this->db->join('ks_user_gear_order_description as gears' , 'a.user_gear_desc_id = orders.user_gear_desc_id AND gears.app_user_id = a.create_user AND orders.order_id = gears.order_id' ,'INNER');
	...
	public function RenterReviewList($where)
	{
		...
		$this->db->join('ks_users as users' , 'a.create_user = users.app_user_id' ,'INNER');
		$this->db->join('ks_user_gear_rent_details as orders' , 'orders.order_id = a.order_id' ,'INNER');
		$this->db->join('ks_user_gear_order_description as gears' , 'gears.user_gear_desc_id = orders.user_gear_desc_id AND gears.app_user_id = a.app_user_id AND orders.order_id = gears.order_id' ,'INNER');
		$this->db->where($where);
		$this->db->group_by('a.order_id');
		$query = $this->db->get();
		return  $query ;
	...
	public function getOwnerReviewOrderList($app_user_id,$where,$limit='',$offset='',$order_by='')
	{
		...
		$this->db->join('ks_users AS b'," b.app_user_id = gears.app_user_id " ,"INNER");
		$this->db->join('ks_users AS users'," users.app_user_id = rent_details.create_user " ,"INNER");
		$this->db->join('ks_cust_gear_reviews AS reivews'," reivews.order_id != rent_details.order_id " ,"RIGHT");
		$this->db->limit($limit,$offset);
		$this->db->group_by('rent_details.order_id');
		$query = $this->db->get();
		return $query ;
