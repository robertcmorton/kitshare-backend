...
;
	...
	public function ModelDetails(){
		...
		if(count($result_arr) > 0 ){
			if($result_arr->model_image != ''){
				$result_arr->model_image = 'http://inferasolz.com/kitshare/'.'admin/uploads/model_images/'.$result_arr->model_image ;
			}
	...
	{
		 $imagePath =  $_SERVER['DOCUMENT_ROOT'].'/kitshare/site_upload/model_images/'.$image;
		$newPath = $_SERVER['DOCUMENT_ROOT'].'/kitshare/site_upload/gear_images/';
		$ext = '.jpg';
		$newName  = time()."-".$ext;
		//die;
	...
	// echo 	$_SERVER['DOCUMENT_ROOT'].'/kitshare/site_upload/model_images/'.$modal_data->model_image ;
		if(!empty($modal_data->model_image)){
			 if (file_exists($_SERVER['DOCUMENT_ROOT'].'/kitshare/site_upload/model_images/'.$modal_data->model_image )) {
...
{   
    ...
    $config = array();
    $config['upload_path'] = $_SERVER['DOCUMENT_ROOT'].'/kitshare/site_upload/gear_images/';;
...
public function resize($image_data) {
	...
	$config['new_image'] = $_SERVER['DOCUMENT_ROOT'].'/kitshare/site_upload/gear_images/new_' . $img;
	$config['width'] = '300';
	$config['height'] = '300';
	...
	public function base64decode()
	{
		...
		$file =  $_SERVER['DOCUMENT_ROOT'].'/kitshare/site_upload/'. uniqid() . '.png';
	...
	public function FAQCatgoryList(){
		...
		if (count($gear_list) > 0 ) {
			foreach ($gear_list as  $value) {
				if ($value->image == '') {
						$value->image = FAQ_IMAGE.'no-image-icon.png';
				}
	...
	public function  GetFaqDetails(){
		...
		if ($gear_list->image == '') {
						$gear_list->image = FAQ_IMAGE.'no-image-icon.png';
		}
	...
	//Address Check END
		...
		if ($data['searchby_fields'] != '') {
			//echo $data['searchby_fields'] ;
			if ($data['searchby_fields'] == 'distance') { 
		...
		if ($app_user_id != '') {
			...
			if ($post_data['insurance_type'] == '1' ) { /// Damage Waiver
				...
				//  $gaer_data = $this->home_model->geratDetails($post_data['user_gear_desc_id']);
			 	...
			 	// print_r($Cart_details->gear_rent_request_to_date);die;
					...
					// print_r($Cart_details);
				 	...
				 	foreach ($Cart_details as  $value) {
				 		...
				 		$rent_summary = array(
				 								...
				 								'beta_discount'=> $beta_discount,
				 								'insurance_fee'=> $gear_insurance_value,
				 								'community_fee'=> $community_fee,
	...
	public function OwnerMail($app_user_id)
	{
		...
		// print_r($Cart_details);die;
		// print_r(date('d-M-Y',strtotime($Cart_details[0]['gear_rent_request_to_date'])));die;
		$mail_content ='<!doctype html>
			...
			$mail_content .='
				...
				<table width="940" border="0" style="margin:0 auto;padding: 0px 10px;">
				  ...
				  <tr>
				    <td>
				     <a href="http://inferasolz.com/kitshare"> <button  style="background-color:#428bca; color:#fff; padding:10px 12px; border:none; margin-bottom:20px">ACCEPT</button> </a>
				     <a href="http://inferasolz.com/kitshare" ><button style="background-color:#ff0000; color:#fff; padding:10px 12px; border:none; margin-bottom:20px">DECLINE</button></a>
				    </td>
				  </tr>
				</table>
...
//				echo $mail_content;die;
	...
	public function  RenterRequestMail($app_user_id='')
	{
		...
		$subject = "RENTER REQUESTS DETAILS FOR DATES";		
		$this->home_model->send_email($sender_mail,$to,$subject,$mail_body);
	}
				...
				$update_cart  = array( 
			 	...
			 	$this->common_model->UpdateRecord($update_cart,"ks_user_gear_rent_details","user_gear_rent_detail_id",$value['user_gear_rent_detail_id']);						 		
					...
					if ($total_hrs < 48) {
						...
						$result = $gateway->paymentMethodNonce()->create($users_details->braintree_token);
						$query = $this->common_model->GetAllWhere('ks_user_gear_payments',array('gear_order_id'=>$order_id));
		...
		$mail_content = '<!doctype html>
						...
						<table width="940" border="0" style="margin:0 auto;padding: 0px 10px">
						  <tbody><tr>
						    <td><h2 style="color:#095cab; font-family:"Gill Sans", "Gill Sans MT", "Myriad Pr", "DejaVu Sans Condensed", Helvetica, Arial, sans-serif; border-bottom:2px solid #095cab; padding-bottom:10px; margin:0">OWNER ADDRESS</h2>
						      ...
						      <p style="font-size:15px; color:#818181; margin:0; padding:5px 0px; font-family:"Gill Sans", "Gill Sans MT", "Myriad Pro", "DejaVu Sans Condense", Helvetica, Arial, sans-serif;">ABN 85 623 435 709</p></td>
						...
						<table class="table table-condensed" width="940" style="margin:25px auto;padding: 0px 10px" cellpadding="0" cellspacing="0">
						  <thead>
						    <tr>
						      <td width="500" style="padding:5px 0px; border-bottom:1px solid #ddd"><strong>Description </strong></td>
						      <td width="150" class="text-center" style="padding:5px 0px; border-bottom:1px solid #ddd"><strong>No of  days </strong></td>
						      <td width="150" class="text-center" style="padding:5px 0px; border-bottom:1px solid #ddd"><strong>Unit Price</strong></td>
						      <td class="text-right" style="padding:5px 0px; border-bottom:1px solid #ddd"><strong>GST</strong></td>
						      <td width="150"class="text-right" style="padding:5px 0px; border-bottom:1px solid #ddd; text-align:right"><strong>Amount AUD</strong></td>
						    </tr>
						  </thead>
						  <tbody>';
				...
				$str .= '<tr>
				      ...
				      <td class="text-right" style="padding:10px 0px; border-bottom:1px solid #ddd">'.number_format((float)$line['gst_amount'], 2, '.', '').'</td>
	...
	public function RenterRejectmailOrder($order_id='')
	{
		...
		$msg= '<!doctype html>
				...
				<tr>
				<td style="font-size:18px; padding-bottom:10px;">We’re sorry to say that the '.$cart_details[0]['app_user_first_name'].'  '.$cart_details[0]['app_user_last_name'].' declined your reservation request.</td>
				</tr>
				<tr>
				...
				<td style="font-size:18px; padding-bottom:10px;"><h2>Thanks,</h2>
	...
	{   
	    //upload an image options
	    $config = array();
	    $config['upload_path'] = $_SERVER['DOCUMENT_ROOT'].'/kitshare/site_upload/checklist/';;
	    $config['allowed_types'] = 'gif|jpg|png|jpeg|svg';
	    $config['max_size']      = '0';
	    $config['overwrite']     = FALSE;
		...
		if ($app_user_id != '') {	
				...
				if (!empty($checklist)) {
					...
					foreach ($checklist as  $value) {
							$checklist[$i]->image = BASE_URL.'site_upload/checklist/'.$value->image ; 
		...
		if ($app_user_id != '') {
			...
			foreach ($Cart_details as  $gear_arr) {
			 ...
			 // 	$gear_details = $query1->row();
				...
				$total_cost_non_gst = $total_cost_ex_gst ;
				$beta_discount = ($total_cost_non_gst*15)/100 ; 
			 	$insurance_fee = $gear_arr['insurance_fee'] ; 
			 	$community_fee = ($total_cost_non_gst*5)/100 ;
				if ($user_details->registered_for_gst != 'Y' ) {
					 $totla_gst = 0  ;
				}else{
	...
	public function Usercheckoutcheck()
	{
		...
		if ($app_user_id != '') {
			...
			$sum = 0 ;
			foreach ($Cart_details as $value) {
					$sum += $value['replacement_value_aud_inc_gst'] ;  
			}
			$query =  $this->common_model->GetAllWhere('ks_settings','');
			$settings =  $query->row();
			$settings->max_replacement_value;
			if($sum > $settings->max_replacement_value){
						$app_user_id = '';
						$response['status'] = 401;
						$response['status_message'] = ' Total Cart replacement value amount should not be grater than '.$settings->max_replacement_value;
						$json_response = json_encode($response);
						echo $json_response;
						exit();
			}
